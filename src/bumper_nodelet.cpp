...
#include <pairs_msgs/msg/obstacle_sectors.hpp>
#include <pairs_msgs/msg/histogram.hpp>

//}

namespace pairs_bumper
{

  using ObstacleSectors = pairs_msgs::msg::ObstacleSectors;
  using Histogram = pairs_msgs::msg::Histogram;
  ...
  class Bumper : public pairs_lib::Node
  {
  public:
    /* Bumper() constructor //{ */
    Bumper(const rclcpp::NodeOptions& options) : pairs_lib::Node("Bumper", options)
    {
      node = this->this_node_ptr();
      clock = node->get_clock();

      /* Load parameters from ROS //{*/
      pairs_lib::ParamLoader pl(node);
      ...
      m_drmgr_ptr = std::make_shared<pairs_lib::DynparamMgr>(node, mutex_drs_params_);
      m_drmgr_ptr->get_param_provider().copyYamls(pl.getParamProvider());

      m_drmgr_ptr->register_param("structuring_element_a", &drs_params_.structuring_element_a, pairs_lib::DynparamMgr::range_t<int>(1, 10));
      m_drmgr_ptr->register_param("structuring_element_b", &drs_params_.structuring_element_b, pairs_lib::DynparamMgr::range_t<int>(1, 10));
      m_drmgr_ptr->register_param("dilate_iterations", &drs_params_.dilate_iterations, pairs_lib::DynparamMgr::range_t<int>(0, 10));
      m_drmgr_ptr->register_param("erode_iterations", &drs_params_.erode_iterations, pairs_lib::DynparamMgr::range_t<int>(0, 10));
      m_drmgr_ptr->register_param("erode_ignore_empty_iterations", &drs_params_.erode_ignore_empty_iterations, pairs_lib::DynparamMgr::range_t<int>(0, 10));
      m_drmgr_ptr->register_param("histogram_n_bins", &drs_params_.histogram_n_bins, pairs_lib::DynparamMgr::range_t<int>(1, 10000));
      m_drmgr_ptr->register_param("histogram_quantile_area", &drs_params_.histogram_quantile_area, pairs_lib::DynparamMgr::range_t<int>(1, 10000));
      m_drmgr_ptr->register_param("max_depth", &drs_params_.max_depth, pairs_lib::DynparamMgr::range_t<double>(0.0, 65.535));
      m_drmgr_ptr->register_param("median_filter_size", &drs_params_.median_filter_size, pairs_lib::DynparamMgr::range_t<int>(1, 100));
      m_drmgr_ptr->register_param("update_rate", &drs_params_.update_rate, pairs_lib::DynparamMgr::range_t<double>(1.0, 100.0));
      ...
      pairs_lib::construct_object(m_depthmap_sh, shopts, "~/depthmap_in");
      pairs_lib::construct_object(m_depth_cinfo_sh, shopts, "~/depth_cinfo_in");
      pairs_lib::construct_object(m_lidar3d_sh, shopts, "~/lidar3d_in");
      pairs_lib::construct_object(m_lidar2d_sh, shopts, "~/lidar2d_in");
      pairs_lib::construct_object(m_lidar1d_down_sh, shopts, "~/lidar1d_down_in");
      pairs_lib::construct_object(m_lidar1d_up_sh, shopts, "~/lidar1d_up_in");

      pairs_lib::PublisherHandlerOptions phopts;
      phopts.node = node;

      // Initialize publishers
      m_obstacles_pub = pairs_lib::PublisherHandler<ObstacleSectors>(phopts, "~/obstacle_sectors");
      m_processed_depthmap_pub = pairs_lib::PublisherHandler<sensor_msgs::msg::Image>(phopts, "~/processed_depthmap");
      m_depthmap_hist_pub = pairs_lib::PublisherHandler<Histogram>(phopts, "~/depthmap_histogram");
      m_lidar3d_processed = pairs_lib::PublisherHandler<sensor_msgs::msg::PointCloud2>(phopts, "~/lidar3d_processed");
      ...
      m_tfm = std::make_unique<pairs_lib::Transformer>(node);
  ...
  private:
    ...
    std::shared_ptr<pairs_lib::DynparamMgr> m_drmgr_ptr;
    std::mutex mutex_drs_params_;
    DynParams_t drs_params_;

    pairs_lib::SubscriberHandler<sensor_msgs::msg::Image> m_depthmap_sh;
    pairs_lib::SubscriberHandler<sensor_msgs::msg::CameraInfo> m_depth_cinfo_sh;
    pairs_lib::SubscriberHandler<sensor_msgs::msg::PointCloud2> m_lidar3d_sh;
    pairs_lib::SubscriberHandler<sensor_msgs::msg::LaserScan> m_lidar2d_sh;
    pairs_lib::SubscriberHandler<sensor_msgs::msg::Range> m_lidar1d_down_sh;
    pairs_lib::SubscriberHandler<sensor_msgs::msg::Range> m_lidar1d_up_sh;
    std::unique_ptr<tf2_ros::Buffer> m_tf_buffer;
    std::unique_ptr<tf2_ros::TransformListener> m_tf_listener_ptr;

    pairs_lib::PublisherHandler<ObstacleSectors> m_obstacles_pub;
    pairs_lib::PublisherHandler<sensor_msgs::msg::Image> m_processed_depthmap_pub;
    pairs_lib::PublisherHandler<Histogram> m_depthmap_hist_pub;
    pairs_lib::PublisherHandler<sensor_msgs::msg::PointCloud2> m_lidar3d_processed;
    ...
    std::unique_ptr<pairs_lib::Transformer> m_tfm;
...
RCLCPP_COMPONENTS_REGISTER_NODE(pairs_bumper::Bumper)
