...
namespace pairs_bumper
{
  ...
  class HistogramDisplayer : public pairs_lib::Node
  {
  public:
    HistogramDisplayer(const rclcpp::NodeOptions& options = rclcpp::NodeOptions()) : pairs_lib::Node("histogram_displayer", options)
    {
      ...
      pairs_lib::SubscriberHandlerOptions shopts(this_node_ptr());
      sh_hist = pairs_lib::SubscriberHandler<pairs_msgs::msg::Histogram>(shopts, "~/histogram", rclcpp::Duration::from_seconds(5.0));

      timer_ = this_node_ptr()->create_wall_timer(std::chrono::milliseconds(33), [this]() {
        if (sh_hist.newMsg())
        {
          const pairs_msgs::msg::Histogram hist_msg = *(sh_hist.getMsg());
          const int hr = 1000;
          const int hc = 1000;
  ...
  private:
    pairs_lib::SubscriberHandler<pairs_msgs::msg::Histogram> sh_hist;
...
RCLCPP_COMPONENTS_REGISTER_NODE(pairs_bumper::HistogramDisplayer)
